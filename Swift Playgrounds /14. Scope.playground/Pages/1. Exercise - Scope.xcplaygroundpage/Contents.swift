...
//answer: varaible foo only exists in the scope of for-loop
...
//the code above can compile because x is defined in global scope, and as a result it can be modified in any sub-scope of global scope as well. However, foo is defined inside for-loop scope, so it can not be accessed or modified outside for-loop scope
/*:
